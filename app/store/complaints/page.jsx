...
export default function StoreComplaintsPage() {
    ...
    useEffect(() => {
        ...
        const fetchComplaints = async () => {
            const response = await fetch(`/api/store/complaints?userId=${encodeURIComponent(user.id)}`, {
                cache: "no-store",
            });
    ...
    const updateComplaint = async (complaintId, status) => {
        ...
        try {
            const response = await fetch(`/api/store/complaints/${complaintId}`, {
                method: "PATCH",
            ...
            setComplaints((prev) => prev.map((item) => (item.id === complaintId ? payload.data : item)));
                ...
                <table className="min-w-full bg-white text-sm">
                    ...
                    <tbody className="divide-y divide-slate-200">
                        ...
                        {filteredComplaints.length === 0 && (
                            <tr>
                                <td colSpan={5} className="py-6 px-4 text-center text-slate-400">No complaints found.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
