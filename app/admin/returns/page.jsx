...
export default function AdminReturnsPage() {
    ...
    const updateReturn = async (returnId, status) => {
        ...
        try {
            ...
            setReturns((prev) =>
                prev.map((item) => (item.id === returnId ? payload.data : item))
            );
    ...
    const exportReturnsCsv = (rows) => {
        ...
        link.href = url;
        link.download = `returns-admin-${new Date().toISOString().slice(0, 10)}.csv`;
            ...
            <div className="flex items-start justify-between gap-4 max-sm:flex-col">
                ...
                <div className="flex items-center gap-3 w-full max-w-xl">
                    ...
                    <button
                        ...
                        onClick={() => {
                            toast.promise(
                                fetch("/api/admin/returns/reminders", { method: "POST" })
                                    .then((res) => res.json())
                                    .then((data) => {
                                        if (!data?.success) throw new Error(data?.message || "Failed to send reminders.");
                                        return data?.message || "Pickup reminders sent.";
                                    }),
                                {
                                    loading: "Sending reminders...",
                                    success: (msg) => msg || "Reminders sent.",
                                    error: (err) => err?.message || "Failed to send reminders.",
                                }
                            );
                        }}
                        className="whitespace-nowrap rounded-md bg-slate-800 px-3 py-2 text-xs text-white hover:bg-slate-900"
                    >
                        Send Reminders
                    </button>
                </div>
                ...
                <table className="min-w-full bg-white text-sm">
                    ...
                    <tbody className="divide-y divide-slate-200">
                        ...
                        {filteredReturns.length === 0 && (
                            <tr>
                                <td colSpan={8} className="py-6 px-4 text-center text-slate-400">No return requests found.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
