    ...
    const linkSections = useMemo(
        () => [
            ...
            },
            {
                ...
                links: [
                    { text: "Home", path: "/" },
                    { text: "Privacy Policy", path: "/legal" },
            ...
            },
            {
                title: "CONTACT",
                links: [
                    { text: contactInfo.email, path: `mailto:${contactInfo.email}`, icon: MailIcon, external: true },
                    { text: contactInfo.phone, path: `tel:${contactInfo.phone}`, icon: PhoneIcon, external: true },
                    { text: contactInfo.address, path: `https://maps.google.com/?q=${encodeURIComponent(contactInfo.mapQuery || contactInfo.address)}`, icon: MapPinIcon, external: true },
                ],
            },
        ],
        [contactInfo]
    ...
    return (
        <footer className="mx-6 bg-white">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-10 border-b border-slate-500/30 text-slate-500">
                    <div>
                        ...
                        <p className="max-w-[410px] mt-6 text-sm">
                            Welcome to Thrift Store, your ultimate destination for budget-friendly secondhand branded clothes and shoes.
                            We bring you quality style in one place.
                        </p>
                        <div className="flex items-center gap-3 mt-5">
                            {socialIcons.map((item, i) => (
                                <a href={item.link} target="_blank" rel="noopener noreferrer" key={i} className="flex items-center justify-center w-10 h-10 bg-slate-100 hover:scale-105 hover:border border-slate-300 transition rounded-full">
                                    <item.icon />
                                </a>
                            ))}
                        </div>
                                ...
                                <ul className="space-y-2.5">
                                    {section.links.map((link, i) => (
                                        <li key={i} className="flex items-center gap-2">
                                            {link.icon && <link.icon />}
                                            {link.external ? (
                                                <a href={link.path} target="_blank" rel="noopener noreferrer" className="hover:underline transition">
                                                    {link.text}
                                                </a>
                                            ) : (
                ...
                <p className="py-4 text-sm text-slate-500">Copyright 2025 &copy; Thrift Store. All Rights Reserved.</p>
            </div>
        </footer>
    );
};
