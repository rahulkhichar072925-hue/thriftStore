...
const Hero = () => {
    ...
    const clothesCategoriesRoute = '/shop?category=Clothes,Topwear,Jacket,Winter%20Jacket,Fashion,Saree'
    const shoesCategoriesRoute = '/shop?category=Shoes,Running%20Shoes,Sneakers,Golf%20Shoes,Heels'
    const currency = process.env.NEXT_PUBLIC_CURRENCY_SYMBOL || '?'

    return (
        <div className='mx-6'>
            <div className='flex max-xl:flex-col gap-8 max-w-7xl mx-auto my-10'>
                <div onClick={() => router.push(clothesCategoriesRoute)} className='relative flex-1 flex flex-col bg-green-200 rounded-3xl xl:min-h-100 group cursor-pointer'>
                    <div className='p-5 sm:p-16'>
                        ...
                        <div className='text-slate-800 text-sm font-medium mt-4 sm:mt-8'>
                            <p>Starts from</p>
                            <p className='text-3xl'>{currency} 599</p>
                        </div>
                        <button onClick={() => router.push(clothesCategoriesRoute)} className='bg-slate-800 text-white text-sm py-2.5 px-7 sm:py-5 sm:px-12 mt-4 sm:mt-10 rounded-md hover:bg-slate-900 hover:scale-103 active:scale-95 transition'>LEARN MORE</button>
                    </div>
                ...
                <div className='flex flex-col md:flex-row xl:flex-col gap-5 w-full xl:max-w-sm text-sm text-slate-600'>
                    <div onClick={() => router.push(shoesCategoriesRoute)} className='flex-1 flex items-center justify-between w-full bg-orange-200 rounded-3xl p-6 px-8 group cursor-pointer'>
                        <div>
                    ...
                    <div onClick={() => router.push('/shop')} className='flex-1 flex items-center justify-between w-full bg-blue-200 rounded-3xl p-6 px-8 group cursor-pointer'>
                        <div>
